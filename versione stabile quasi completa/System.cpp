...
namespace System {
	...
	Warehouse::Warehouse(double initCash_desk) :
		month { 0 }, cash_desk { initCash_desk } { fill_data(); }
	...
	{
	    if (ord.Order::orderValue() < cashDesk())
	        return true;
	    return false;
	}
	...
	void Warehouse::check_situation() {
	    //Per prima cosa, aggiungo gli ordini che posso effettuare al vettore orderList
	    for (int i=0; i < orderTemp.size(); ++i) {
	        if (acceptOrder(orderTemp[i])) {
		        orderList.push_back(orderTemp[i]); //Se ho soldi a sufficienza, metto l'ordine nel vettore orderList
		        orderTemp.erase(orderTemp.begin() + i); //e lo tolgo dal vettore temporaneo con gli ordini considerati
		    }
		}
		Order tmp;
	...
	void Warehouse::fill_data() {
		...
		while (ist_info >> component_id >> component_name >> delivery_time
			...
			components_infos.push_back(component_info { component_id, component_name, delivery_time, c_price }); // store
		ist_info.close();
