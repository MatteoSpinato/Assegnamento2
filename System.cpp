...
namespace System {
	...
	void Warehouse::addOrder(orderInit fMain) {
        ...
        if (OrderValue(newOrder)) //OrderValue stabilisce se ho soldi a sufficienza per procedere con l'ordine
		    orderList.push_back(newOrder); //Allora aggiungo alla lista di ordini il nuovo ordine
		else
		    orderTemp.push_back(newOrder); //Aggiungo questo ordine in un vettore di ordini da considerare più avanti, quando avrò cassa a sufficienza
		//newOrder.PrintOrder();
	}

    //Ritorna true se la somma di denaro che un determinato ordine necessita può essere sostenuta dalla cassa.
    bool Warehouse::OrderValue(Order ord) {
        double somma_totale = 0;
        for (int i=0; i<ord.orderComponentsSize(); ++i)
            somma_totale = somma_totale + ord.orderComponents[i].////////////final price; da sistemare questo punto
        somma_totale = somma_totale * ord.quantityOrdered();
        if (somma_totale < Warehouse::cashDesk())
            return true;
        return false;
    }
